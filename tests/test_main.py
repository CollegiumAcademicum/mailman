...
class TestBot(unittest.TestCase):
    ...
    @patch("scripts.handlers.bot_info", {"team_id": "test_team_id"})
    ...
    @patch("scripts.handlers.driver", new_callable=MagicMock)
    @patch("scripts.handlers.VISIBLE_CHANNEL_GROUPS", {})
    ...
    @patch("scripts.handlers.driver", new_callable=MagicMock)
    @patch("scripts.handlers.WHITELIST", ["channel_id_1"])
    @patch("scripts.handlers.VISIBLE_CHANNEL_GROUPS", {"Group 1": ["id1"]})
    def test_conversation_flow(self, mock_driver):
        ...
        with patch(
            "scripts.handlers.resolve_targets",
        ...
        mock_driver.reset_mock()
        with (
            patch("scripts.handlers.bot_info", {"bot_username": "testbot"}),
            patch("scripts.handlers.log_broadcast") as mock_log,
        ):
    ...
    def test_file_attachment_flow(self, mock_driver):
        ...
        with patch("scripts.handlers.bot_info", {"bot_username": "testbot"}):
    ...
    def test_invalid_channel_selection(self, mock_driver):
        ...
        with patch(
            "scripts.handlers.resolve_targets", return_value=([], [], ["invalid1", "invalid2"])
        ):
